from fastapi import APIRouter, HTTPException

from models.session import (
    EventsAccepted,
    EventsIn,
    SessionCreate,
    SessionEndOut,
    SessionOut,
)
from store import add_events, create_session, end_session, get_events

router = APIRouter(prefix="/sessions", tags=["sessions"])


@router.post("", response_model=SessionOut, status_code=201)
def create(body: SessionCreate) -> SessionOut:
    record = create_session(body.user_id, body.lift)
    return SessionOut(**record)


@router.post("/{session_id}/events", response_model=EventsAccepted)
def post_events(session_id: str, body: EventsIn) -> EventsAccepted:
    total = add_events(session_id, body.events)
    if total is None:
        raise HTTPException(status_code=404, detail="session not found")
    return EventsAccepted(accepted=len(body.events), total_for_session=total)


@router.post("/{session_id}/end", response_model=SessionEndOut)
def end(session_id: str) -> SessionEndOut:
    session = end_session(session_id)
    if session is None:
        raise HTTPException(status_code=404, detail="session not found")
    return SessionEndOut(
        session_id=session["session_id"],
        ended_at=session["ended_at"],
        event_count=len(get_events(session_id)),
    )
